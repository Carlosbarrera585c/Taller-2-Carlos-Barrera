...
class controllerClass {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $rsp = modelClass::putNew($_POST['textId'], $_POST['textUsuId'], $_POST['textCredId']);
            if ($rsp === true) {
    ...
    public function update() {
        if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
                ...
                if (count($certificate) > 0) {
                    ...
                    if (is_array($data)) {
                        if (count($data) > 0) {
                            ...
                            $args['textCredId'] = $data[0]['credencial_id'];
                        }
                    } else {
                        $args['error'] = $data;
                        viewClass::renderHTML('error.php', $args);
            ...
            $data['usuarioId'] = $_POST['textUsuId'];
            $data['credencial_id'] = $_POST['textCredId'];
